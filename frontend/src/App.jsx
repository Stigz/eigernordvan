...
export default function App() {
  ...
  const handleBookingSubmit = async (event) => {
    ...
    try {
      ...
      const isEditing = Boolean(bookingEditId);
      const response = await fetch(isEditing ? `${apiBaseUrl}/bookings/${bookingEditId}` : `${apiBaseUrl}/bookings`, {
        method: isEditing ? "PUT" : "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      setBookingStatus({
        state: "success",
        message: `${isEditing ? "Updated" : "Saved"} ${payload.status} booking · ${payload.nights} nights · CHF ${payload.estimate_total.toFixed(2)}`,
      });
      setBookingEditId("");
  ...
  const handleEditBooking = (booking) => {
    ...
    setBookingForm({
      ...
      status: booking.status === "open_override" ? "booked" : booking.status,
      guest_name: booking.guest_name || "",
      day_km: String(booking.day_km || ""),
      notes: booking.notes || "",
    });
  ...
  const handleDeleteBooking = async (booking) => {
    const shouldDelete = window.confirm(`Delete booking from ${booking.start_date} to ${booking.end_date}?`);
    if (!shouldDelete) {
      return;
    }

    try {
      const response = await fetch(`${apiBaseUrl}/bookings/${booking.id}`, { method: "DELETE" });
      const payload = await response.json();
      if (!response.ok) {
          ...
          <div className="panel-grid">
            <section className="card">
              <header>
                ...
                <h1>{bookingEditId ? "Edit booking" : "Create booking"}</h1>
              ...
              <form className="form" onSubmit={handleBookingSubmit}>
                ...
                <div className="form-actions">
                  <button className="submit" type="submit" disabled={bookingStatus.state === "loading"}>
                    {bookingStatus.state === "loading" ? "Saving..." : bookingEditId ? "Update booking" : "Create booking"}
                  </button>
                  {bookingEditId && (
                    <button className="cancel" type="button" onClick={handleCancelBookingEdit}>
                      Cancel edit
                    </button>
                  )}
                </div>
              ...
              {bookingStatus.state !== "idle" && (
                <div className={`status ${bookingStatus.state}`} aria-live="polite">
                  ...
                  {calendarCells.map((cell) => (
                    <button
                      ...
                      onClick={() =>
                        setBookingForm((prev) => ({
                          ...prev,
                          start_date: prev.start_date || cell.iso,
                          end_date: prev.end_date || formatDateISO(addDays(parseIsoDate(cell.iso), 1)),
                        }))
                      }
                    >
                      <span>{cell.day}</span>
